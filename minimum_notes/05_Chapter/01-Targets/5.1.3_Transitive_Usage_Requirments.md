...
As we decussed before, interface properties are only used to propagate the properties further down the chain(to the next destination target), and the sourc target won't use them in its build process.  
